    ...
    while (1) {
        ...
        if(packets_from_pool == packets_used) {
            ...
            if (packets_from_pool == 0) {
                ...
                SCLogDebug("Pool has %d packets available", packets_from_pool);
        ...
        int packets_received = ipc_populate_packets(ptv->ipc, &current_packets[packets_used], packets_available);

        SCLogDebug("Received %d packets from ipc", packets_received);
...
TmEcode ReceiveIpcThreadInit(ThreadVars *tv, const void *initdata, void **data)
{
    ...
    if (ConfGet("ipc.server", &tmpserver) != 1) {
        ...
        if(!create_ipc_client(tmpserver, &ptv->ipc)) {
    ...
    *data = (void *)ptv;
    ...
    SCLogInfo("IPC Source Ready, connected to %s, with batch size of %lu", ptv->server_name, ptv->allocation_batch);
