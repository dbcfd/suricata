...
impl NFSState {
    ...
    pub fn process_request_record_lookup<'b>(
        ...
        if let Ok( (_, lookup) ) = parse_nfs3_request_lookup(r.prog_data) {
    ...
    fn process_reply_record<'b>(&mut self, r: &RpcReplyPacket<'b>) -> u32 {
        ...
        match self.requestmap.remove(&r.hdr.xid) {
            ...
            _ => {
                SCLogDebug!(
                    "REPLY: xid {} NOT FOUND. GAPS? TS:{} TC:{}",
                    r.hdr.xid,
                    self.ts_ssn_gap,
                    self.tc_ssn_gap
                );
    ...
    pub fn process_read_record<'b>(
        ...
        if nfs_version == 2 {
            let size = if let Ok( (_, ref attr) ) = parse_nfs2_attribs(reply.attr_blob) {
                attr.asize
            } else {
                0
            };
        ...
        SCLogDebug!(
            "REPLY {} to procedure {} blob size {} / {}: chunk_left {}",
            r.hdr.xid,
            NFSPROC3_READ,
            r.prog_data.len(),
            reply.count,
                            ...
                            if rpc_phdr.procedure == NFSPROC3_WRITE {
                                ...
                                match parse_rpc(cur_i) {
                                    Ok( (remaining, ref rpc_record) ) => {
                                        if let Ok( (_, ref nfs_request_write) ) =parse_nfs3_request_write(
    ...
    pub fn parse_udp_ts<'b>(&mut self, input: &'b [u8]) -> u32 {
        ...
        if input.len() > 0 {
            let res = parse_rpc_udp_request(input);
            if let Ok( (_, ref rpc_record)) = res {
                self.is_udp = true;
    ...
    pub fn parse_udp_tc<'b>(&mut self, input: &'b [u8]) -> u32 {
        ...
        if input.len() > 0 {
            let res = parse_rpc_udp_reply(input);
            if let Ok( (_, ref rpc_record) ) = res {
                self.is_udp = true;
                status |= self.process_reply_record(rpc_record);
            } else {
...
pub fn nfs_probe_udp(i: &[u8], direction: u8) -> i8 {
    if direction == STREAM_TOCLIENT {
        if let Ok( (_, ref rpc) ) = parse_rpc_udp_reply(i) {
            if i.len() >= 32
                && rpc.hdr.msgtype == 1
                && rpc.reply_state == 0
                && rpc.accept_state == 0
